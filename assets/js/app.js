...
const GOOGLE_REVIEWS_CACHE_MAX_ENTRIES = 300;
const PHOTO_LOOKUP_CACHE_MAX_ENTRIES = 200;
let exploreCards = [];
...
const LOCATION_LABEL_CACHE_MAX_ENTRIES = 1000;
...
let pendingAssistantMessageId = null;
  ...
  function pruneExpiringMap(cache, maxEntries) {
    const now = Date.now();

    for (const [key, value] of cache.entries()) {
      if (value && Number.isFinite(Number(value.expiresAt))) {
  ...
  function buildAboutCampsiteText(camp, narrative) {
    const listingName = String(camp?.name || 'This campsite').trim();
    const listingLoc = String(camp?.loc || 'Location unavailable').trim();
    const descriptionSentence = firstSentence(narrative?.description || camp?.descriptionText || '');
    const sentenceOne = descriptionSentence || `${listingName} is a campsite option for this trip.`;
    const sentenceTwo = `It is located in ${listingLoc}.`;
    const sentenceThree = `The surrounding area and trip logistics are tied to ${listingLoc}.`;
  ...
  function extractHardConstraintHints(queryText) {
    const text = String(queryText || '').toLowerCase();
    return {
      dogFriendly: /\bdog\b|\bpet\b/.test(text),
      rv: /\brv\b|motorhome|hookups?/.test(text),
      tent: /\btent\b|primitive/.test(text),
      waterfront: /waterfront|lake|river|beach|ocean/.test(text),
      budget: /\$\s*\d+|under\s+\d+|budget|cheap|afford/.test(text)
    };
  }

  function hasResolvedHardConstraint(intent, hints) {
    if (!intent || typeof intent !== 'object') {
      return false;
    }

    const constraints = intent.constraints && typeof intent.constraints === 'object' ? intent.constraints : {};
    if (hints.dogFriendly && constraints.dogFriendly !== true) {
      return false;
    }
    if (hints.rv && constraints.rv !== true) {
      return false;
    }
    if (hints.tent && constraints.tent !== true) {
      return false;
    }
    if (hints.waterfront && constraints.waterfront !== true) {
      return false;
    }
    if (hints.budget && !(Number.isFinite(Number(constraints.maxPrice)) && Number(constraints.maxPrice) > 0)) {
      return false;
    }
  ...
  function getRequiredClarificationQuestions(intent, queryText) {
    ...
    if (confidence < 0.45) {
      questions.push('Can you confirm your exact destination and your top two must-have campsite features?');
    }

    const hints = extractHardConstraintHints(queryText);
    if ((hints.dogFriendly || hints.rv || hints.tent || hints.waterfront || hints.budget) && !hasResolvedHardConstraint(intent, hints)) {
      questions.push('I detected hard requirements in your request. Which are mandatory versus nice-to-have?');
    }
  ...
  async function runSearch(queryOverride) {
    ...
    try {
      ...
      const sourceStatusText = 'Searching live Recreation.gov and NPS inventory...';
      setPendingAssistantMessage(sourceStatusText);
  ...
  function getCachedLocationLabel(lat, lon) {
    pruneExpiringMap(locationLabelCache, LOCATION_LABEL_CACHE_MAX_ENTRIES);
  ...
  async function fetchRidbExploreBatch(query, limit = 6) {
    ...
    return rows.map(normalizeRidbFacilityRecord);
  }
  ...
  function mapRidbToCampCard(facility, index, availableSites, campsiteSummary = null) {
    ...
    const state = normalized.stateCode || 'US';
    const city = normalized.city || '';
    const lat = normalized.lat;
    const lon = normalized.lon;
    const facilityDirections = normalized.directions;
    const summary = campsiteSummary && typeof campsiteSummary === 'object' ? campsiteSummary : summarizeRidbCampsites([]);
    const descriptionText = buildProviderDescription({
      FacilityDescription: normalized.description,
      FacilityUseFeeDescription: normalized.feeDescription,
      FacilityDirections: normalized.directions
    }, ['FacilityDescription', 'FacilityUseFeeDescription', 'FacilityDirections']);
    ...
    return {
      ...
      name: normalized.name || 'Recreation.gov Campground',
      type: `${normalized.typeDescription || 'Federal'} · Recreation.gov${summary.count > 0 ? ` · ${summary.count} campsites` : ''}`,
      sourceProvider: 'recreation_gov',
      providerSchemaVersion: normalized.schemaVersion,
      recreationCampgroundId: String(normalized.providerId || '').trim(),
      bookingUrl: buildRecreationBookingUrl(normalized.providerId, getBookingWindow()),
  ...
  function mapNpsToCampCard(campground, index, availableSites) {
    ...
    const state = normalized.stateCode || 'US';
    const city = normalized.city || 'National Park';
    const npsLat = normalized.lat;
    const npsLon = normalized.lon;
    const recIdFromReservationLink = extractRecreationCampgroundId(normalized.reservationUrl || '');
    const descriptionText = normalized.description;
    const tags = buildAmenityTags({
      descriptionText,
      facilityText: [normalized.description, normalized.directions].filter(Boolean).join(' '),
      campsiteSummary: null,
      baseTags: ['NPS', 'Park campground', recIdFromReservationLink ? 'Recreation booking link' : 'Info listing']
    });

    return {
      id: `nps-${normalized.providerId || index}`,
      name: normalized.name || 'NPS Campground',
      type: 'National Park Service · Campground',
      sourceProvider: 'nps',
      providerSchemaVersion: normalized.schemaVersion,
      recreationCampgroundId: recIdFromReservationLink || '',
      bookingUrl: String(normalized.reservationUrl || '').trim(),
      loc: `${city}, ${state}`,
      cityName: city,
      stateCode: state,
      lat: npsLat,
  ...
  function dedupeInventoryCards(cards) {
    const seen = new Set();
    return cards.filter(card => {
      const byUrl = String(card?.bookingUrl || '').trim().toLowerCase();
      const byNameLoc = `${String(card?.name || '').trim().toLowerCase()}::${String(card?.loc || '').trim().toLowerCase()}`;
      const key = byUrl || byNameLoc;
      if (!key) {
        return true;
      }
      if (seen.has(key)) {
  ...
  function renderDetailListingFields(camp) {
    ...
    const mapContainerEl = document.getElementById('d-location-map');
    ...
    if (descEl) {
      descEl.textContent = buildAboutCampsiteText(camp, narrative);
    }
    ...
    if (mapContainerEl) {
      if (hasCoords) {
        const mapQuery = encodeURIComponent(`${camp.lat},${camp.lon}`);
        const embedUrl = `https://maps.google.com/maps?q=${mapQuery}&z=12&output=embed`;
        mapContainerEl.classList.remove('empty');
        mapContainerEl.innerHTML = `<iframe title="Map for ${escapeHtml(listingName)}" loading="lazy" referrerpolicy="no-referrer-when-downgrade" src="${embedUrl}"></iframe>`;
      } else {
        mapContainerEl.classList.add('empty');
        mapContainerEl.innerHTML = '<span class="map-label">Map preview unavailable</span>';
      }
    }
  ...
  async function fetchGoogleReviews(camp) {
    ...
    const now = Date.now();
    pruneExpiringMap(googleReviewsCache, GOOGLE_REVIEWS_CACHE_MAX_ENTRIES);
    ...
    const reviewRequestPromise = (async () => {
      ...
      if (controller) {
        timeoutId = setTimeout(() => controller.abort(), GOOGLE_REVIEWS_REQUEST_TIMEOUT_MS);
      }

      try {
        const res = await apiClient.getGoogleReviews(query, controller ? { signal: controller.signal } : undefined);
        const payload = await res.json();
        ...
        const didTimeout = String(err?.name || '').toLowerCase() === 'aborterror';
        const fallbackPayload = {
          enabled: false,
          source: 'fallback',
          reviews: [],
          error: didTimeout
            ? 'Google reviews timed out. Try opening this listing again in a moment.'
    ...
    googleReviewsInFlight.set(cacheKey, reviewRequestPromise);
    try {
      return await reviewRequestPromise;
    } finally {
  ...
  async function fetchFallbackCampgroundPhotos(camp) {
    ...
    const cacheKey = query.toLowerCase();
    pruneExpiringMap(photoLookupCache, PHOTO_LOOKUP_CACHE_MAX_ENTRIES);
    if (photoLookupCache.has(cacheKey)) {
      return photoLookupCache.get(cacheKey);
    }

    try {
      const res = await apiClient.getMediaPhotos(query);
      if (!res.ok) {
        photoLookupCache.set(cacheKey, []);
        return [];
      }

      const payload = await res.json();
      const photos = [...new Set((Array.isArray(payload?.photos) ? payload.photos : []).map(normalizeImageUrl).filter(Boolean))];
      photoLookupCache.set(cacheKey, photos);
      return photos;
    } catch (err) {
      photoLookupCache.set(cacheKey, []);
      return [];
  ...
  function downloadCampinApp() {
    const url = 'https://example.com/campin-app';
    const tab = window.open(url, '_blank', 'noopener,noreferrer');
    if (!tab) {
      alert('Popup blocked. Please allow popups to open the app download page.');
    }
  }
